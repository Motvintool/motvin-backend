...
async function processSource(src) {
  ...
  try {
    ...
    console.log(`  ✓ Added ${collectionIcons.length} logos from ${src.sourceName}`);
  } catch (e) {
...
async function main() {
  ...
  for (const [id, data] of collectionsMap) {
    ...
    fs.writeFileSync(path.join(collectionPath, 'metadata.json'), JSON.stringify(data.metadata, null, 2));
    fs.writeFileSync(path.join(collectionPath, 'icons.json'), JSON.stringify(data.icons, null, 2));
    
    collectionsList.push(data.metadata);
    totalIcons += data.metadata.total;
    
    sourcesOutput[id] = { license: data.metadata.license };
  }
  ...
  fs.writeFileSync(path.join(outputDir, 'collections.json'), JSON.stringify({
