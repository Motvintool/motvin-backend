...
export class LoaderService implements OnModuleInit {
  ...
  async getMetadata(collectionId: string) {
    ...
    try {
      const path = join(this.dataRoot, "icons", collectionId, "metadata.json");
      const data = await readFile(path, "utf-8");
  ...
  async getCollection(collectionId: string) {
    ...
    try {
      ...
      for (const icon of icons) {
        const searchStr =
          `${icon.name} ${(icon.tags || []).join(" ")}`.toLowerCase();
        let matched = false;
  ...
  async calculateStats() {
    ...
    for (const collection of this.collectionsData.collections) {
      ...
      try {
        ...
        this.logger.warn(
          `Failed to load icons for ${collection.id}: ${error.message}`,
        );
      }
    }
