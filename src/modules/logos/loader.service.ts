...
export class LoaderService implements OnModuleInit {
  ...
  async getCollection(collectionId: string) {
    ...
    try {
      ...
      const categoryKeywords = {
        Technology: ['tech', 'software', 'computer', 'digital', 'code', 'web', 'internet', 'app', 'system', 'data', 'cloud', 'cyber'],
        Social: ['social', 'network', 'chat', 'connect', 'community', 'forum'],
        Finance: ['bank', 'money', 'pay', 'crypto', 'finance', 'capital', 'wallet', 'invest', 'wealth', 'trading'],
        Commerce: ['shop', 'store', 'cart', 'retail', 'buy', 'ecommerce', 'market', 'trade', 'deal'],
        Entertainment: ['movie', 'film', 'music', 'game', 'play', 'media', 'studio', 'art', 'cinema', 'radio', 'tv'],
        Automotive: ['car', 'auto', 'vehicle', 'motor', 'drive', 'transport', 'wheels'],
        Food: ['food', 'drink', 'coffee', 'restaurant', 'eat', 'meal', 'cafe', 'bakery', 'kitchen', 'recipe'],
        Healthcare: ['health', 'medical', 'care', 'medicine', 'hospital', 'clinic', 'wellness', 'pharmacy', 'dental'],
        Education: ['school', 'university', 'learn', 'study', 'academy', 'college', 'tutor', 'knowledge', 'student'],
        Travel: ['travel', 'trip', 'hotel', 'flight', 'tour', 'journey', 'holiday', 'vacation', 'explore'],
        RealEstate: ['home', 'house', 'property', 'estate', 'land', 'realty', 'mortgage', 'rent', 'building', 'architecture'],
        Fashion: ['fashion', 'clothing', 'wear', 'style', 'apparel', 'boutique', 'design', 'trend', 'garment'],
        Sports: ['sport', 'game', 'fitness', 'team', 'athletic', 'gym', 'workout', 'active'],
        Construction: ['build', 'construction', 'tool', 'contractor', 'engineer', 'architect', 'structure'],
        Energy: ['energy', 'power', 'green', 'eco', 'solar', 'electric', 'nature', 'enviroment'],
        Media: ['news', 'press', 'broadcast', 'journal', 'magazine', 'publish', 'print'],
        Agency: ['agency', 'consult', 'group', 'partner', 'solution', 'service', 'firm', 'studio', 'manage'],
      };
      ...
      for (const logo of logos) {
        const searchStr = `${logo.name} ${(logo.tags || []).join(' ')}`.toLowerCase();
        let matched = false;
  ...
  async calculateStats() {
    ...
    const categoryList = ['Technology', 'Social', 'Finance', 'Commerce', 'Entertainment', 'Automotive', 'Food', 'Healthcare', 'Education', 'Travel', 'RealEstate', 'Fashion', 'Sports', 'Construction', 'Energy', 'Media', 'Agency'];
    ...
    for (const collection of this.collectionsData.collections) {
      ...
      try {
        ...
        this.logger.warn(`Failed to load logos for ${collection.id}: ${error.message}`);
      }
    }
