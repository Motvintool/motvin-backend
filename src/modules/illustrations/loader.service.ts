...
export class LoaderService implements OnModuleInit {
  ...
  async getMetadata(collectionId: string) {
    ...
    try {
      const filePath = join(this.dataRoot, 'illustrations', collectionId, 'metadata.json');
      const data = await readFile(filePath, 'utf-8');
      const metadata = JSON.parse(data);
      this.cacheService.setCollection(cacheKey, metadata);
      return metadata;
    } catch (error) {
      this.logger.error(`Failed to load metadata for ${collectionId}`, error.message);
      return null;
    }
  }
  ...
  async getCollection(collectionId: string) {
    ...
    if (cached) {
      this.logger.debug(`Cache HIT for illustration collection: ${collectionId}`);
      return cached;
    }

    this.logger.debug(`Cache MISS for illustration collection: ${collectionId}`);

    try {
      const filePath = join(this.dataRoot, 'illustrations', collectionId, 'icons.json');
      const startTime = Date.now();
      const data = await readFile(filePath, 'utf-8');
      const items = JSON.parse(data);

      const categoryKeywords: Record<string, string[]> = {
        People:       ['person', 'people', 'human', 'man', 'woman', 'girl', 'boy', 'team', 'crowd', 'family'],
        Business:     ['business', 'work', 'office', 'meeting', 'finance', 'money', 'startup', 'corporate', 'deal'],
        Technology:   ['tech', 'computer', 'phone', 'device', 'digital', 'app', 'software', 'data', 'robot', 'ai'],
        Nature:       ['nature', 'tree', 'plant', 'flower', 'mountain', 'sea', 'earth', 'eco', 'green', 'forest'],
        Health:       ['health', 'medical', 'doctor', 'hospital', 'fitness', 'sport', 'wellness', 'yoga', 'exercise'],
        Education:    ['education', 'school', 'book', 'study', 'learn', 'student', 'teacher', 'science', 'knowledge'],
        Travel:       ['travel', 'trip', 'journey', 'map', 'plane', 'city', 'adventure', 'tourism', 'hotel'],
        Lifestyle:    ['lifestyle', 'home', 'food', 'cooking', 'music', 'art', 'shopping', 'fashion', 'relax'],
        Abstract:     ['abstract', 'geometric', 'shape', 'pattern', 'colorful', 'gradient', 'wave', 'minimal'],
      };
      ...
      for (const item of items) {
        const searchStr = `${item.name} ${(item.tags || []).join(' ')}`.toLowerCase();
        let matched = false;
      ...
      this.logger.log(`Loaded ${items.length} illustrations from ${collectionId} in ${elapsed}ms`);

      this.cacheService.setCollection(collectionId, items);
      return items;
    } catch (error) {
      this.logger.error(`Failed to load illustration collection ${collectionId}`, error.message);
      return null;
    }
  }
  ...
  async calculateStats() {
    ...
    const categoryList = ['People', 'Business', 'Technology', 'Nature', 'Health', 'Education', 'Travel', 'Lifestyle', 'Abstract', 'Others'];
    ...
    for (const collection of this.collectionsData.collections) {
      ...
      if (this.sourcesData?.[collection.id]) {
        ...
        byLicense.set(license, (byLicense.get(license) || 0) + collection.total);
      }
      ...
      try {
        ...
        this.logger.warn(`Failed to load illustrations for ${collection.id}: ${e.message}`);
      }
    }
